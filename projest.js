...
function toggleStyle(id) {
    ...
    buttons.forEach(btn=>{
        document.getElementById(btn).classList.remove('bg-blue-600','text-white');
        document.getElementById(btn).classList.add('text-[#64748B]','border-gray-200');
    });
...
function setStatusInAllCards(id,status){

    const cards = allCardSection.querySelectorAll('.flex.justify-between');

    cards.forEach(card=>{

        const title = card.querySelector('.mobile').innerText;
...
document.addEventListener("click",function(e){
    ...
    if(interviewBtn){
        ...
        if(!InterviewList.find(x=>x.id===id)){
            InterviewList.push(job);
        }

        setStatusInAllCards(id,"Interview");
    ...
    if(rejectedBtn){
        ...
        if(!RejectedList.find(x=>x.id===id)){
            RejectedList.push(job);
        }

        setStatusInAllCards(id,"Rejected");
...
function renderFilteredData(list,status,color){
    ...
    list.forEach(item=>{
        ...
        <h3 class="mobile text-[#002C5C] text-[24px] font-bold">
        ${item.mobile}
        </h3>

        <p class="React text-[#64748B] font-medium">
        ${item.react}
        </p>
        ...
        <p class="Build text-[#64748B] text-sm mb-6">
        ${item.build}
        </p>
